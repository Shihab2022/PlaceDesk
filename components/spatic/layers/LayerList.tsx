...
import { CATEGORIES, formatCount, type ComputedLayer } from "../data";

export interface LayerListProps {
  layers: ComputedLayer[];
  activeKeys: Set<string>;
  activeId: string | null;
  addableKeys: Set<string>;
  cityLabel: string;
...
function LayerMenu({
  ...
  layer: ComputedLayer;
  onClose: () => void;
  actions: Omit<LayerListProps, "layers" | "activeKeys" | "activeId" | "addableKeys" | "cityLabel">;
}) {
    ...
    <div ref={rootRef} className="relative flex min-h-0 flex-col">
      ...
      >
        {props.layers.map((l) => {
          ...
          const selected = props.activeId === l.id;
          return (
            <div key={l.id} role="listitem" className="relative">
              ...
              {menuFor === l.id && (
                <LayerMenu layer={l} onClose={() => setMenuFor(null)} actions={actions} />
              )}
            </div>
          );
        })}
      ...
      <p className="sr-only" aria-live="polite">
        {props.activeId
          ? `${props.layers.find((l) => l.id === props.activeId)?.label ?? ""} layer configuration open for ${props.cityLabel}`
          : ""}
      </p>
    </div>
  );
